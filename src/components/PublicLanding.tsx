...
import { Wallet, Globe, Zap, ShieldCheck, ArrowRight, Lock, Rocket, ChevronRight, Star, Layers, Terminal, X, Play, Loader2, Monitor, ArrowRightLeft, CheckCircle2, Server, ShieldAlert, FileText, Activity, Coins, Sparkles, Network, TrendingUp, Cpu, Database, BarChart3, Fingerprint, Users, Landmark, ArrowLeftRight } from 'lucide-react';
...
export const PublicLanding: React.FC<PublicLandingProps> = ({ onStart, onRemittance, isLoggedIn }) => {
      ...
      <nav className="fixed top-0 left-0 w-full z-50 bg-brand-950/80 backdrop-blur-md border-b border-brand-900 px-6 py-4">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            <button onClick={onRemittance} className="hidden md:flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-action-500 hover:text-white transition-colors">
                <ArrowLeftRight className="w-3 h-3" /> Send Money
            </button>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 items-center relative z-10">
          <div className="lg:col-span-8">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4">
              ...
              <button onClick={onRemittance} className="w-full sm:w-auto px-12 py-6 bg-white text-brand-950 rounded-sm font-bold text-sm uppercase tracking-[0.3em] hover:bg-brand-50 transition-all flex items-center justify-center gap-2">
                <ArrowLeftRight className="w-4 h-4" /> Send Money Now
              </button>
            </div>
          </div>
