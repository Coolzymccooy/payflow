    ...
    try {
      ...
      const sessionPromise = ai.live.connect({
        ...
        config: {
          ...
          systemInstruction: 'You are the Payflow OS Voice Oracle. You are an expert in institutional finance and treasury operations. Educate users on the "Bilateral Ledger Mirroring" protocol which enables T+0 atomic settlement by bypassing legacy SWIFT networks. Explain the "Nexus Bridge" as a secure HSM-level handshake for linking tier-1 bank enclaves like Stanbic, Halifax, and Chase. Be executive, technical, and concise. Mention how "Corridor Hedging" protects capital against FX volatility in the SSA/UK corridors via high-sensitivity reactivity nodes.'
        },
        callbacks: {
          onopen: () => {
            setIsActive(true);
      ...
      {error ? (
        <div className="p-8 text-center space-y-6 animate-in fade-in">
             ...
             <button 
                ...
                className="w-full py-3 bg-brand-950 text-white font-bold uppercase text-[10px] tracking-widest rounded-sm hover:bg-brand-900 transition-all border border-brand-800"
             >
