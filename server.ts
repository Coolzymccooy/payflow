...
const authenticate = (req: any, res: any, next: NextFunction) => {
  try {
    const umbrellaId = req.headers["x-umbrella-id"] as string | undefined;
    if (!umbrellaId) return res.status(401).json({ error: "AUTH_REQUIRED" });
...
app.patch("/api/regions/:id/toggle", authenticate, (req, res) => {
  ...
  if (region) region.status = region.status === "ACTIVE" ? "INACTIVE" : "ACTIVE";
  res.json({ success: true, region });
});

app.post("/api/regions/:id/rails", authenticate, (req, res) => {
  const { id } = req.params;
  const { railName } = req.body as { railName?: string };
  const region = db.regions.find((r) => r.id === id);
  if (region && railName && !region.rails.includes(railName)) region.rails.push(railName);
  res.json(db.regions);
});
...
app.delete("/api/regions/:id/rails/:rail", authenticate, (req, res) => {
  ...
  const region = db.regions.find((r) => r.id === id);
  if (region) region.rails = region.rails.filter((r: string) => r !== rail);
